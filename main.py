...
from models import UserPreference, SavedJob
...
async def search_jobs(request: Request, db: Session = Depends(get_db)):
    ...
    if not pref or not pref.positions or not pref.location:
        return HTMLResponse("<div class='error-msg'>Please set and save your position and location preferences first.</div>")
        
    search_term = pref.positions
    if pref.fields:
        search_term += f" {pref.fields}"
        
    fetched_jobs = fetch_jobs(search_term=search_term, location=pref.location, distance_miles=50, results_wanted=20)
    ...
    for j in fetched_jobs:
        ...
        if not existing_job:
            ...
            try:
                ...
                db.rollback()
                existing_job = db.query(SavedJob).filter(SavedJob.job_id == str(j['job_id'])).first()
                if existing_job:
                    saved_jobs_list.append(existing_job)
        else:
            saved_jobs_list.append(existing_job)
    
    return templates.TemplateResponse(request=request, name="partials/job_results.html", context={"jobs": saved_jobs_list, "pref": pref})
...
async def get_job_detail(request: Request, id: int, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(request=request, name="partials/job_detail.html", context={"job": job})
